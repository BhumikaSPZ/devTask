    ...
    var callback = function (mutationsList, observer) {
        for (var mutation of mutationsList) {
        	...
        	if (submitBtn && dropdownEle){
				...
				var price = infoTxt.querySelector('span')?.innerText;
				if (plan == "lifetime") {
            ...
            if (infoTxt && (infoTxt.innerHTML.includes('30-day') || infoTxt.innerHTML.includes('7 days'))) {
                ...
                if (!infoTxt.querySelector('.today-total')) {
                    infoTxt.innerHTML = isLifeTime ? 'One-Time Fee. 30-Day Money-Back Guarantee.<p class="total-value font-medium">Total:<span class="total-value-price font-medium">'  +  price +  '</span></p>' :
                        '<span class="font-medium today-total">TODAY’S Total:</span><span class="total-value font-medium">$0 for 7 days</span><br><p class="font-light">After 7 days: ' + price + ' (billed monthly)</p>';
