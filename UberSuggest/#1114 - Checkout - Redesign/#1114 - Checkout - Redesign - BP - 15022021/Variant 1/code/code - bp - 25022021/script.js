...
var tier = getParams('tier');
var plan = getParams('plan');
if (plan == 'monthly') {
	document.querySelector('body').classList.add('isMonthly');
}
else if (plan == 'lifetime') {
	document.querySelector('body').classList.add('isLifeTime');
}
else {
	document.querySelector('body').classList.add('isLifeTime');
}
if (tier == 'tier1') {
	document.querySelector('body').classList.add('isIndividual');
}
else if (tier == 'tier2') {
	document.querySelector('body').classList.add('isBusiness');
}
else if (tier == 'tier3') {
	document.querySelector('body').classList.add('isEnterprise');
}
else {
	document.querySelector('body').classList.add('isIndividual');
}
...
var orderReviewHtml = '<h3 class="title">Your Order Review</h3><div class="order-info"><p class="font-light info-text">.One-Time Fee. 30-Day Money-Back Guarantee</p></div>';
    ...
    function initSelector() {
        var setHeadingEleInterval = setInterval(function () {
            ...
            if (headingEle) {
                ...
                // Spz badge
                headingEle?.parentElement?.parentElement?.PreviousElementSibling?.firstElementChild?.children[0]?.classList.add('spz-control-badge');
                // Form Title
				...
				var target =  document.querySelector('.selector-value>div:first-child');
	            target.children[0].children[0].classList.add('selecter-text');
            }
        }, 500);
    }
    initSelector();
    ...
    var callback = function (mutationsList, observer) {
        for (var mutation of mutationsList) {
            ...
            if (!mainHeader && document.querySelector('.spz-wrapper')) {
                ...
                var formWrapper = document.querySelector('.spz-form-wrapper');
                var planSelecter = document.querySelector('.spz-left-block');
                var spzForm = document.querySelector('.spz-form');
	            formWrapper.insertBefore(planSelecter,spzForm);
                
                // Change form title 
                var spzFormTitle = document.querySelector('.spz-form-title');
                if (plan == 'monthly') {
					spzFormTitle.innerHTML = '7-Day Free Trial';
				}
				if (plan == 'lifetime') {
					spzFormTitle.innerHTML = 'Lifetime Access';
				}
				else if (document.body.classList.contains('isLifeTime')) {
					spzFormTitle.innerHTML = 'Lifetime Access<div></div>';
				}
                
                var paypalBtnImg = document.querySelector('.paypal-btn img');
	            paypalBtnImg.setAttribute('src', 'https://res.cloudinary.com/spiralyze/image/upload/f_auto/Ubsersuggest/1114%20-%20Checkout%20-%20Redesign/paypal_btn_img.png');
            ...
            if (leftBlock) {
	            ...
	            if (plan == 'monthly') {
					target = leftBlock.children[3];
					if(target.innerText.includes('7 days')) {
	                	target.classList.add('switch-plan-text');
					}
				}
				if (plan == 'lifetime') {
					target = leftBlock.children[2];
					if(target.innerText.includes('30-day')) {
	                	target.classList.add('switch-plan-text');
					}
                ...
                // var BilledPlan = document.querySelector('.spz-order-review');
                // document.querySelector('.spz-right-block .paypal-btn').before(BilledPlan);
                running = true;
                setTimeout(function () { running = false; }, 100);
            }
            ...
            (!document.querySelector('.spz-switch-plan-link') && leftBlock?.childElementCount == 6)) {
                ...
                leftBlock.children[1].children[0].className = "get-plan-text";
                tagret.children[1].className = "change-plan-link";
                tagret.children[1].innerHTML = '<span>Switch to Lifetime Plan</span><span class="font-light">to Save 90%</span>';
            ...
            if (submitBtn) {
                if (plan == "lifetime" && document.body.classList.contains('isLifeTime')) {
	            	submitBtn.children[0].innerHTML = 'GET LIFETIME ACCESS' ;	
	            }
                if (plan == "monthly" && document.body.classList.contains('isMonthly')) {
            ...
            var infoTxt = document.querySelector('.switch-plan-text div');
            if (infoTxt && (infoTxt.innerHTML.includes('30-day') || infoTxt.innerHTML.includes('7 days'))) {
                ...
                // Add Total Text
                if (!infoTxt.querySelector('.info-text')) {
                    infoTxt.innerHTML = isLifeTime ? '<p class="font-light info-text">One-Time Fee. 30-Day Money-Back Guarantee</p>' :
                        '<p class="font-light info-text">After 7 days: ' + price + ' (billed monthly)</p>';
            ...
            if (mutation.type == "childList" && (mutation.addedNodes.length == 1 || mutation.removedNodes.length == 1) && mutation.target.classList.contains('spz-left-block')) {
                if (leftBlock?.childElementCount == 6) {
                    ...
                    infoTxt.innerHTML = '<p class="font-light info-text">After 7 days: ' + price + ' (billed monthly)</p>';
                ...
                if (leftBlock?.childElementCount == 5) {
                    ...
                    infoTxt.innerHTML = '<p class="font-light info-text">One-Time Fee. 30-Day Money-Back Guarantee</p>';
